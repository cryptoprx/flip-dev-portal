...
async function createSubmission(data) {
  const { rows } = await sql`
    INSERT INTO submissions (ext_id, name, version, description, author, author_email, category, icon, type, permissions, api_version, blob_url, manifest_json)
    VALUES (${data.ext_id}, ${data.name}, ${data.version}, ${data.description}, ${data.author}, ${data.author_email || null}, ${data.category}, ${data.icon || 'puzzle'}, ${data.type || 'sidebar'}, ${data.permissions || []}, ${data.api_version || '1.0'}, ${data.blob_url}, ${JSON.stringify(data.manifest_json || {})})
    RETURNING *
  `;
  return rows[0];
}
